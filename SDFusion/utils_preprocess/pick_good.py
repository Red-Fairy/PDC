...
for line in tqdm(lines):
    ...
    src = os.path.join(root, 'pcd', f'{object_id}_{part_id}-{instance_id}.ply')
    dst = os.path.join(dst_root_pcd, f'{object_id}_{part_id}.ply')
    os.system(f'cp {src} {dst}')
    src = os.path.join(root, 'meshes_canonical', f'{object_id}_{part_id}-{instance_id}.obj')
    dst = os.path.join(dst_root_mesh_canonical, f'{object_id}_{part_id}.obj')
    os.system(f'cp {src} {dst}')
    src = os.path.join(root, 'meshes', f'{object_id}_{part_id}-{instance_id}.obj')
