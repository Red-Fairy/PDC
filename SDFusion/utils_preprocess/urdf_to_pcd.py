...
def urdf_to_pcd(urdf_path, save_path):
    ...
    urdf_obj = loader.load(urdf_path)
...
def generate_all_pcds():
    save_root = '/mnt/data-rundong/PartDiffusion/dataset/part_ply'
    ...
    for category in categories:
        ...
        for i, (obj_id, part_id) in tqdm(enumerate(obj_part_tuple)):
            ...
            urdf_path = os.path.join('/raid/haoran/Project/data/partnet_all_annotated_new/annotation', obj_id, f'remove_single_part_wo-{category}-link_{part_id}.urdf')
            save_path = os.path.join(save_root, category, f"{obj_id}_{part_id}.ply")
            urdf_to_pcd(urdf_path, save_path)

def generate_specified_pcd():
    root = './annotation_files'
    # save_root = './remove_all_parts'
    dirnames = [x for x in os.listdir(root) if os.path.isdir(os.path.join(root, x))]
    for dirname in dirnames:
        print(dirname)
        urdf_path = os.path.join(root, dirname, 'remove_all.urdf')
        save_path = os.path.join(root, dirname, '.ply')
        urdf_to_pcd(urdf_path, save_path)

if __name__ == '__main__':
    generate_specified_pcd()