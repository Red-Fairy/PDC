...
def bbox_to_mesh(bbox, color=None, render_all=False):
    # device='cuda'
    device=bbox.device

    # extract meshes from bbox
    bs = bbox.shape[0]
    nc = 1
    ...
    for i in range(nimg_to_render):
        bbox_i = bbox[i].detach().cpu().numpy()
        verts_i = bbox_i
        faces_i = np.array([[i,j,k] for i in range(8) for j in range(8) for k in range(8)])
        verts_i = torch.from_numpy(verts_i).float().to(device)
        faces_i = torch.from_numpy(faces_i.astype(np.int64)).to(device)
        text_i = torch.ones_like(verts_i).to(device)
        if color is not None:
...
def render_bbox(mesh_renderer, bbox, level=0.02, color=None, render_imsize=256, render_all=False):
    """ 
        shape of bbox:
        - bs, 8, 3
