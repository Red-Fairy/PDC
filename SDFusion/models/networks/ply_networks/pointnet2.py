...
class PointNet4ScalePrediction(nn.Module):
    def __init__(self,normal_channel=False,hidden_dim=256,scale_range=2.):
        super(PointNet4ScalePrediction, self).__init__()
        in_channel = 6 if normal_channel else 3
        self.normal_channel = normal_channel
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.2, nsample=32, in_channel=in_channel, mlp=[64, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.4, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256], group_all=False)
        ...
        self.bn2 = nn.BatchNorm1d(hidden_dim)
        self.drop2 = nn.Dropout(0.4)
        self.scale_pred = nn.Linear(hidden_dim, 1)
    ...
    def forward(self, xyz):
        ...
        scale = torch.sigmoid(self.scale_pred(x)) * self.scale_range
