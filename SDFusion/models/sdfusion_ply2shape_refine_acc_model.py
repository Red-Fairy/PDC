    ...
    def get_current_visuals(self):
        ...
        if hasattr(self, 'ply_translation'):
            ...
            for i, mesh in enumerate(meshes):
                visuals_dict['part_scale'][i] = torch.max(self.part_extent[i]).item() / np.max(mesh.extents) if self.opt.scale_model == 'max_extent' else \
                    (torch.cumprod(self.part_extent[i], dim=-1)[-1].item() / np.cumprod(mesh.extents)[-1]) ** (1/3)
