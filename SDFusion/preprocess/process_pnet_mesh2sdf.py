...
def process_obj(obj_file, isosurface_dir, sdf_cmd, mcube_cmd, reduce=1, sdf_res=64, h5_file=None):
    ...
    num_sample = 65 ** 3
    bandwidth = 0.1 # snet
    sdf_res = sdf_res  #! sdf resolution
    expand_rate = 1.3
    iso_val = 0.003 # snet
    max_verts = 16384
    g=0.0 # snet
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cat', type=str, default='handle')
    args = parser.parse_args()
    cat_list = ['button', 'door', 'drawer', 'knob', 'lid']

    for cat in cat_list:
        args.cat = cat
        basedir = f'/mnt/seagate12t/GAPartNet/test/part_manifold_normalized/{args.cat}_manifold'
        out_basedir = f'/mnt/seagate12t/GAPartNet/test/PartSDF.normalized/{args.cat}'
        os.makedirs(out_basedir, exist_ok=True)

        obj_files = glob.glob(f'{basedir}/*.obj')
        for obj_file in obj_files:
            print(f'[*] processing {obj_file}')
            out_file = obj_file.replace(basedir, out_basedir).replace('.obj', f'_sdf_res_{sdf_res//reduce}.h5')
            process_obj(obj_file, isosurface_dir, sdf_cmd, mcube_cmd, reduce=reduce, sdf_res=sdf_res, h5_file=out_file)
        print(f'mesh {cat} to sdf: done!')
