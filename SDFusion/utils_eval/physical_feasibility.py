...
def get_physical_loss(sdf, ply, scale, part_translation, 
    ...
    print(f'Collision loss: {loss_collision.item()}, Contact loss: {loss_contact.item()}')
    return loss_collision < 1e-4 and loss_contact < 1e-4

def physical_feasibility(mesh: trimesh.Trimesh, pcd: torch.Tensor,
                         res: int=128, padding: float=0.2, margin: float=1/256,
                         grid_length=1/128, steps=5, device='cuda:0'):
    '''
    mesh: trimesh mesh
    pcd: point cloud, (N, 3)
    '''
    ...
    # build a grid
    x = torch.linspace(-grid_length, grid_length, steps).to(device) 
    y = torch.linspace(-grid_length, grid_length, steps).to(device) 
    z = torch.linspace(-grid_length, grid_length, steps).to(device) 
...
def main():
    ...
    parser.add_argument('--test_root', type=str, default='./logs/slider_drawer-ply2shape-plyrot-scale3-lr0.00001/test_250000_rotate0.0_scale3.0_eta0.0_steps50_volume_mobility_margin256-haoran/meshes_canonical')
